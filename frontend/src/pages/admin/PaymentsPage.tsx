  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-semibold text-ink tracking-tight">Payments</h2>
      ...
      <div className="flex flex-wrap items-end gap-3">
          ...
          {METHODS.map((m) => (
            <button
              ...
              className={`px-3 py-1 rounded ${
                method === m.value ? "bg-slate-900 text-white" : "text-ink-muted hover:bg-gray-100"
              }`}
            >
              {m.label}
            </button>
        ...
        <div>
          ...
          <input
            ...
            className="rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500"
          />
        </div>
        <div>
...
function Card({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
      ...
      <div className="text-3xl font-semibold text-ink tracking-tight mt-2">{children}</div>
    </div>
  );
}
